...
void Menu::InsertSidebarSearch() {
    ...
    if (!Ship_IsCStringEmpty(CVarGetString(menuEntries["Settings"].sidebarCvar, ""))) {
        curIndex = GetVectorIndexOf(
            menuEntries["Settings"].sidebarOrder, CVarGetString(menuEntries["Settings"].sidebarCvar, "")
        );
    }
    menuEntries["Settings"].sidebarOrder.insert(
        menuEntries["Settings"].sidebarOrder.begin() + searchSidebarIndex, "Search"
    );
...
void Menu::RemoveSidebarSearch() {
    uint32_t curIndex = GetVectorIndexOf(
        menuEntries["Settings"].sidebarOrder, CVarGetString(menuEntries["Settings"].sidebarCvar, "General")
    );
...
void Menu::UpdateWindowBackendObjects() {
    Fast::WindowBackend runningWindowBackend =
        static_cast<Fast::WindowBackend>(Ship::Context::GetRawInstance()->GetWindow()->GetWindowBackend());
...
Menu::Menu(
    const std::string& cVar, const std::string& name, uint8_t searchSidebarIndex_, UIWidgets::Colors defaultThemeIndex_
)
    : GuiWindow(cVar, name)
    , searchSidebarIndex(searchSidebarIndex_)
...
bool ModernMenuSidebarEntry(std::string label) {
    ...
    window->DrawList->AddRectFilled(
        pos - style.FramePadding, pos + labelSize + style.FramePadding,
        ImGui::GetColorU32(
...
uint32_t Menu::DrawSearchResults(std::string& menuSearchText) {
    ...
    ImGui::SetNextWindowSizeConstraints(
        { ImGui::GetContentRegionAvail().x / 2, 0 },
        { ImGui::GetContentRegionAvail().x / 2, ImGui::GetContentRegionAvail().y }
    );
    if (ImGui::BeginChild(
            "Search Results Col 1", { ImGui::GetContentRegionAvail().x / 2, 0 }, ImGuiChildFlags_AutoResizeY,
        ...
        switch (widget.type) {
            ...
            case WIDGET_CVAR_CHECKBOX: {
                ...
                if (UIWidgets::CVarCheckbox(
                        UIWidgets::WrappedText(widget.name.c_str(), width).c_str(), widget.cVar, *options
                    ))
                {
                    if (widget.callback != nullptr) {
                        widget.callback(widget);
            ...
            case WIDGET_VIDEO_BACKEND: {
                ...
                if (UIWidgets::Combobox(
                        "Renderer API (Needs reload)", &configWindowBackend, availableWindowBackendsMap, options
                    ))
                {
                    Ship::Context::GetRawInstance()->GetConfig()->SetInt(
                        "Window.Backend.Id", (int32_t) (configWindowBackend)
                    );
                    Ship::Context::GetRawInstance()->GetConfig()->SetString(
            ...
            case WIDGET_CVAR_COLOR_PICKER: {
                ...
                UIWidgets::CVarColorPicker(
                    widget.name.c_str(), widget.cVar, options->defaultValue, options->useAlpha, modifiers,
                    options->color
                );
            } break;
            case WIDGET_SEARCH: {
...
void Menu::DrawElement() {
    ...
    if (popout != popped) {
        if (popout) {
            ...
            ImGui::SetNextWindowSize(
                { static_cast<float>(windowWidth), static_cast<float>(windowHeight) }, ImGuiCond_Always
            );
    ...
    ImGui::PushStyleColor(
        ImGuiCol_WindowBg, ImVec4(0, 0, 0, CVarGetFloat(CVAR_SETTING("Menu.BackgroundOpacity"), 0.85f))
    );
    ...
    window->DrawList->AddRectFilled(
        pos, pos + ImVec2 { menuSize.x, 4 }, ImGui::GetColorU32({ 255, 255, 255, 255 }), true, style.WindowRounding
    );
    ...
    // Increase sidebar width on larger screens to accomodate people scaling their
    // menus.
    ...
    window->DrawList->AddRectFilled(
        pos, pos + ImVec2 { 4, sectionHeight - style.FramePadding.y * 2 }, ImGui::GetColorU32({ 255, 255, 255, 255 }),
    ...
    if (headerSearch && menuSearchText.length() > 0) {
        ...
        ImGui::BeginChild(
            "searchSeparator", ImVec2(ImGui::GetContentRegionAvail().x / 2, 20),
